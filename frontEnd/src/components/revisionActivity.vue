            ...
            <template v-for="(item,index) in computedActivity">
              <v-list-tile :key="index">
                ...
                <v-list-tile-content>
                  ...
                  <v-list-tile-sub-title > {{item.project.name}}   -{{item.user.computerName}}</v-list-tile-sub-title>
                </v-list-tile-content>
              </v-list-tile>
              <v-divider v-bind:class="'projectDivider'" :key="index" :inset="true" v-bind:style="{ borderColor: item.project.color}"></v-divider>
            </template>
          </v-list>
        </v-card>
      </v-flex>
  ...
  computed: {
    computedActivity() {
        ...
        for (let item of this.activity){
           ...
           console.log(this.projects.find(x => x.id === item.projectId))
           newList.push(item)
        }
