...
await db.run(`CREATE TABLE IF NOT EXISTS projects (id INTEGER PRIMARY KEY AUTOINCREMENT , name TEXT)`)
...
for (let project of projects){
    ...
    let exist = await db.get(query )
    console.log(exist)
    if(!exist){
        await db.run(`INSERT INTO projects (name) VALUES ("${project.ProjectName}")`)
    }
